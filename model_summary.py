...
def last7Days(place,collection):
    user_obj = session.get('user', None)
    name = user_obj['hotel']
    ...
    if place == "-- All --":
        total_reviews = collection.find(
                    {'Name': name, 'City': city, 'Sentiment': {'$ne': 3}, 'Date': {'$gte': unix_time_7days, '$lt': unix_time_today}}).count()
        postive_reviews = collection.find({'Name': name, 'City': city, 'Sentiment':1,'Date': {'$gte': unix_time_7days, '$lt': unix_time_today}}).count()
        negative_reviews = collection.find({'Name': name, 'City': city, 'Sentiment':2,'Date': {'$gte': unix_time_7days, '$lt': unix_time_today}}).count()
        neutral_reviews = collection.find({'Name': name, 'City': city, 'Sentiment':0,'Date': {'$gte': unix_time_7days, '$lt': unix_time_today}}).count()
    else:
        total_reviews = collection.find(
            {'Name': name, 'Place': place, 'City': city, 'Sentiment': {'$ne': 3},
             'Date': {'$gte': unix_time_7days, '$lt': unix_time_today}}).count()
    ...
    if total_reviews ==0:
        ...
        CSI /= total_reviews * 2
        CSI *= 100.0
    data.append({"CSI": round(CSI, 2), "total_reviews": total_reviews,
                 "positive_reviews": postive_reviews, "negative_reviews": negative_reviews})
    return data


def presentMonth(place,collection):
    ...
    if place == "-- All --":
        total_reviews = collection.find(
                    {'Name': name, 'City': city, 'Sentiment': {'$ne': 3}, 'Date': {'$gte': unix_time_month_first, '$lt': unix_time_today}}).count()
        postive_reviews = collection.find({'Name': name, 'City': city, 'Sentiment':1,'Date': {'$gte': unix_time_month_first, '$lt': unix_time_today}}).count()
        negative_reviews = collection.find({'Name': name, 'City': city, 'Sentiment':2,'Date': {'$gte': unix_time_month_first, '$lt': unix_time_today}}).count()
        neutral_reviews = collection.find({'Name': name, 'City': city, 'Sentiment':0,'Date': {'$gte': unix_time_month_first, '$lt': unix_time_today}}).count()
    else:
        total_reviews = collection.find(
            {'Name': name, 'Place': place,'City': city, 'Sentiment': {'$ne': 3},
             'Date': {'$gte': unix_time_month_first, '$lt': unix_time_today}}).count()
        postive_reviews = collection.find({'Name': name,'Place': place, 'City': city, 'Sentiment': 1,
                                           'Date': {'$gte': unix_time_month_first, '$lt': unix_time_today}}).count()
        negative_reviews = collection.find({'Name': name, 'Place': place,'City': city, 'Sentiment': 2,
                                            'Date': {'$gte': unix_time_month_first, '$lt': unix_time_today}}).count()
    ...
    if total_reviews == 0:
        ...
        CSI /= total_reviews * 2
        CSI *= 100.0
    data.append({"CSI": round(CSI, 2), "total_reviews": total_reviews,"positive_reviews": postive_reviews, "negative_reviews": negative_reviews})
    return data


def lastMonth(place,collection):
    dt = dtm.datetime(year=int(time.strftime("%Y")), month=int(time.strftime("%m")), day=1)
    unix_time_month_first = int(time.mktime(dt.timetuple()))
...
def totalReviews(place,collection):
    user_obj = session.get('user', None)
    name = user_obj['hotel']
    city = user_obj['City_Name']
    data = []
...
def getSummaryData(place, year, startdate, enddate):
    ...
    lastMonthScore = lastMonth(place=place,collection=collection)
    totalRev = totalReviews(place=place,collection=collection)
